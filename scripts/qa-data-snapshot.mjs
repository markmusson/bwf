...
for (const [id, v] of stands) {
  console.log(
    `  ${id.padEnd(10)} total=${v.total}  taken=${v.taken}  donors=${v.count}`,
  );
}
const orphans = seats.filter(
  (s) =>
    ![
      "south",
      "west",
...
console.log(
  "  raisedPence:",
  stats.raisedPence,
  `(£${(stats.raisedPence / 100).toFixed(2)})`,
);
...
for (const g of wall.slice(0, 5)) {
  console.log(
    `  - ${g.seat.slug}  donors=${g.donors}  tributes=${g.tributes.length}`,
  );
}
