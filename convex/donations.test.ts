...
describe("donations.createDraft", () => {
  ...
  test("flat £10 floor: £9.99 fails on every stand, £10 passes", async () => {
    const t = convexTest(schema, modules);
    for (const stand of ["south", "hollies", "wyatt"]) {
        ...
        t.run((ctx) =>
          _createDraftForTest(
            ...
            baseArgs({
              ...
              seatId,
              amountPence: 999,
              stripeSessionId: `cs_${stand}_low`,
            }),
          ),
        ),
      ).rejects.toMatchObject({ data: "amount_below_minimum" });
  ...
  test("flat £10 floor: an upward bump (£25) is still accepted", async () => {
    const t = convexTest(schema, modules);
    const seatId = await t.run((ctx) =>
      ctx.db.insert("seats", {
        stand: "south",
    ...
    const result = await t.run((ctx) =>
      _createDraftForTest(
        ...
        baseArgs({
          ...
          seatId,
          amountPence: 2_500,
          stripeSessionId: "cs_bump",
        }),
      ),
    );
    const donation = await t.run((ctx) => ctx.db.get(result.donationId));
    expect(donation?.amountPence).toBe(2_500);
  });
